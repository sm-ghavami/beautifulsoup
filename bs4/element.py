...
class Tag(PageElement):
    ...
    def get_own_text(self):
        return ''.join([tag for tag in iter(self.contents) if not isinstance(tag, Tag)])
    ...
    def select(self, selector, _candidate_generator=None, limit=None):
        ...
        if self._select_debug:
            print('for selector "%s" tokens are "%s"' % (selector, tokens))
        current_context = [self]
        if self._select_debug:
            print('for selector "%s" context is "%s"' % (selector, current_context))
        ...
        for index, token in enumerate(tokens):
            ...
            if ":" in token or "." in token or "#" in token or ("[" in token and "]" in token):
                tag_name, sels = re.split(r"[:\.#\[]", token, 1)
                sels = token[len(tag_name):]
                m = self.attribselect_re.match(sels)
                if m is not None:
                    # Attribute selector
                ...
                if sels is not None and len(sels) > 0 :
                    while True:
                        ...
                        if op == '#':
                            ...
                            def id_matches(tag):
                                if tag_name is not None and len(tag_name) > 0:
                                    return tag.get('id', None) == tag_id and tag.name == tag_name
                                return tag.get('id', None) == tag_id

                            checkers.append(id_matches)
                            ...
                            def classes_match(candidate):
                                if tag_name is not None and len(tag_name) > 0:
                                    return sel_name.issubset(candidate.get('class', [])) and candidate.name == tag_name
                                return sel_name.issubset(candidate.get('class', []))

                            checkers.append(classes_match)

                        elif op == ':' and not self.quoted_colon.search(sels):
                            # Pseudo-class
                            tag_name_, pseudo = sels.split(':', 1)
                            if tag_name_ != '':
                                raise ValueError(
                                    "Invalid select rule.")
                            ...
                            if pseudo_type == 'nth-of-type':
                                if tag_name == '':
                                    raise ValueError(
                                        "A nth-of-type must be prefixed with a tag name.")
                                try:
                                ...
                                checkers.append(Counter(pseudo_value).nth_child_of_type)
                            elif pseudo_type == 'nth-child':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                    pseudo_value = int(pseudo_value)
                                except:
                                ...
                                class Counter(object):
                                    ...
                                    def nth_child(self, tag):
                                        self.count += 1
                                        if self.count == self.destination and (len(_copy_of_tag_name)>0 and tag.name == _copy_of_tag_name):
                                            return True
                                        elif self.count == self.destination and len(_copy_of_tag_name) == 0:
                                            return True
                                        else:
                                            return False

                                checkers.append(Counter(pseudo_value).nth_child)
                            elif pseudo_type == 'lt':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                ...
                                checkers.append(Counter(pseudo_value).lt)
                            elif pseudo_type == 'lte':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                    pseudo_value = int(pseudo_value)
                                ...
                                checkers.append(Counter(pseudo_value).lte)
                            elif pseudo_type == 'gt':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                ...
                                checkers.append(Counter(pseudo_value).gt)
                            elif pseudo_type == 'gte':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                    pseudo_value = int(pseudo_value)
                                ...
                                checkers.append(Counter(pseudo_value).gte)
                            elif pseudo_type == 'eq':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                try:
                                ...
                                checkers.append(Counter(pseudo_value).eq)
                            elif pseudo_type == 'contains':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                ...
                                class Checker(object):
                                    ...
                                    def contains(self, tag):
                                        if self.destination.lower() in tag.get_text().lower() and (len(_copy_of_tag_name)>0 and tag.name == _copy_of_tag_name):
                                            return True
                                        elif self.destination.lower() in tag.get_text().lower() and len(_copy_of_tag_name) == 0:
                                            return True
                                        else:
                                            return False

                                checkers.append(Checker(pseudo_value).contains)

                            elif pseudo_type == 'containsOwn':
                                _copy_of_tag_name = tag_name
                                tag_name = ""
                                ...
                                class Checker(object):
                                    ...
                                    def contains(self, tag):
                                        if self.destination.lower() in tag.get_own_text().lower() and (len(_copy_of_tag_name)>0 and tag.name == _copy_of_tag_name):
                                            return True
                                        elif self.destination.lower() in tag.get_own_text().lower() and len(_copy_of_tag_name) == 0:
                                            return True
                                        else:
                                            return False
                            ...
                            elif pseudo_type == 'matches':
                                _copy_of_tag_name = tag_name
                                tag_name = ""

                                class Checker(object):
                                    def __init__(self, destination):
                                        self.regex = re.compile(destination,re.UNICODE | re.MULTILINE)

                                    def contains(self, tag):
                                        if self.regex.match(tag.get_text()) and (len(_copy_of_tag_name)>0 and tag.name == _copy_of_tag_name):
                                            return True
                                        elif self.regex.match(tag.get_text()) in tag.get_text().lower() and len(_copy_of_tag_name) == 0:
                                            return True
                                        else:
                                            return False
                                ...
                                class Checker(object):
                                    ...
                                    def contains(self, tag):
                                        if self.regex.match(tag.get_own_text()) and (len(_copy_of_tag_name)>0 and tag.name == _copy_of_tag_name):
                                            return True
                                        elif self.regex.match(tag.get_own_text()) and len(_copy_of_tag_name) == 0:
                                            return True
                                        else:
                                            return False
                        ...
                        if len(sel_path) <= 2: break
                        sels = sels[len(sel_path[1])+1:]
            else:
                    ...
                    if len(checkers) > 0:
                        ...
                        try:
                            for checker in checkers:
                                result = result and checker(candidate)
                        except StopIteration:
                            # The checker has decided we should no longer
                            # run the generator.
                            break
                    if len(checkers) == 0 or result:
                        if self._select_debug:
